...
struct FragmentationTests {
    ...
    func maxSizedFileTransferSurvivesReassembly() async throws {
        ...
        let filePacket = PilltalkFilePacket(
            fileName: "limit.bin",
        ...
        let packet = try #require(
            signer.signPacket(PilltalkPacket(
...
extension FragmentationTests {
    ...
    private final class CaptureDelegate: PilltalkDelegate, @unchecked Sendable {
        private let lock = NSLock()
        private var _publicMessages: [(peerID: PeerID, nickname: String, content: String)] = []
        private var _receivedMessages: [PilltalkMessage] = []
        ...
        var receivedMessages: [PilltalkMessage] {
    ...
    private func makeLargePublicPacket(senderShortHex: PeerID, size: Int) -> PilltalkPacket {
    ...
    private func fragmentPacket(_ packet: PilltalkPacket, fragmentSize: Int, fragmentID: Data? = nil, pad: Bool = true) -> [PilltalkPacket] {
