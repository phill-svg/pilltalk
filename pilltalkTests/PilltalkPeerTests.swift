...
@Suite("PilltalkPeer Tests")
struct PilltalkPeerTests {
    ...
    func connectionStatePriorityIsCorrect() {
        ...
        let bluetooth = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "A", isConnected: true, isReachable: true)
        let mesh = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "A", isConnected: false, isReachable: true)
        var nostr = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "A", isConnected: false, isReachable: false)
        nostr.favoriteStatus = mutual
        let offline = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "A", isConnected: false, isReachable: false)
    ...
    func displayNameAndOfflineIconUseDerivedState() {
        ...
        var peer = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "", isConnected: false, isReachable: false)
    ...
    func mutualFavoriteOfflinePeerShowsNostrIcon() {
        ...
        var peer = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "Peer", isConnected: false, isReachable: false)
