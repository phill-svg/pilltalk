...
final class MockBLEService: NSObject {
    ...
    var sentMessages: [(message: PilltalkMessage, packet: PilltalkPacket)] = []
    var sentPackets: [PilltalkPacket] = []
    var connectedPeers: Set<PeerID> = []
    var messageDeliveryHandler: ((PilltalkMessage) -> Void)?
    var packetDeliveryHandler: ((PilltalkPacket) -> Void)?
    ...
    private func deliverLocalEcho(_ message: PilltalkMessage) {
    ...
    func simulateIncomingMessage(_ message: PilltalkMessage) {
        delegate?.didReceiveMessage(message)
    ...
    func simulateIncomingPacket(_ packet: PilltalkPacket) {
        // Process through the actual handling logic
        if let message = PilltalkMessage(packet.payload) {
