...
private final class MockChatTransportEventContext: ChatTransportEventContext {
    ...
    private(set) var handledPublicMessages: [PilltalkMessage] = []
    ...
    func handlePrivateMessage(_ message: PilltalkMessage) {
    ...
    var peersByID: [PeerID: PilltalkPeer] = [:]
    ...
    func unifiedPeer(for peerID: PeerID) -> PilltalkPeer? { peersByID[peerID] }
...
struct ChatTransportEventCoordinatorContextTests {
    ...
    func didConnectAndDisconnect_manageSessionsStableIDsAndReadReceipts() async {
        ...
        context.peersByID[peerID] = PilltalkPeer(peerID: peerID, noisePublicKey: noiseKey, nickname: "alice")
