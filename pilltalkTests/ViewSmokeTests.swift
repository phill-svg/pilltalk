...
struct ViewSmokeTests {
    ...
    func verificationViews_renderCoreBranches() throws {
        ...
        let myQR = MyQRView(qrString: "pilltalk://verify?name=alice&npub=npub1test")
        let qrCode = QRCodeImage(data: "pilltalk://verify?hello=world", size: 96)
    ...
    func geohashAndTextMessageViews_renderCoreBranches() {
        ...
        let truncatableMessage = PilltalkMessage(
        ...
        let paymentMessage = PilltalkMessage(
            sender: viewModel.nickname,
    ...
    func messageRows_snapshotDeliveryStatusForSwiftUIDiffing() {
        // Regression: `PilltalkMessage` is a reference type mutated in place
