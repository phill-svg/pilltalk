...
struct NostrProtocolTests {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedPillTalk.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID),
        ...
        // Parse PillTalk payload
        #expect(content.hasPrefix("pilltalk1:"))
        let base64url = String(content.dropFirst("pilltalk1:".count))
        let packetData = try #require(Self.base64URLDecode(base64url))
        let packet = try #require(PilltalkPacket.from(packetData), "Failed to decode pilltalk packet")
    ...
    @Test func ackRoundTripNIP44V2_ReadReceipt() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedPillTalk.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID),
