...
private final class MockChatNostrContext: ChatNostrContext {
    ...
    private(set) var appendedGeohashMessages: [(message: PilltalkMessage, geohash: String)] = []
    ...
    func handlePublicMessage(_ message: PilltalkMessage, powBits: Int) { handledPublicMessages.append(message) }
    func checkForMentions(_ message: PilltalkMessage) { mentionCheckedMessageIDs.append(message.id) }
    func sendHapticFeedback(for message: PilltalkMessage) { hapticMessageIDs.append(message.id) }
...
struct ChatNostrCoordinatorContextTests {
    ...
    func handleGiftWrap_routesEmbeddedPrivateMessageAndDeduplicates() async throws {
        ...
        let sender = try NostrIdentity.generate()
        let embedded = try #require(NostrEmbeddedPillTalk.encodePMForNostrNoRecipient(
            content: "psst",
            messageID: "gm-1",
...
struct GeoPresenceTrackerTests {
    ...
    func notificationCooldown_stampsGeohashOnceWindowElapses() async throws {
        ...
        let placeholder = PilltalkMessage(
            id: event.id,
            sender: "seed",
            content: "seed",
            timestamp: Date(),
