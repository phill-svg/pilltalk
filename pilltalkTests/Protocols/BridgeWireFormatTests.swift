...
struct BridgeWireFormatTests {
    ...
    // MARK: - PilltalkMessage bridged flag
    ...
    @Test func bridgedFlagSurvivesCodableRoundTrip() throws {
        ...
        let decoded = try JSONDecoder().decode(PilltalkMessage.self, from: data)
    ...
    @Test func legacyJSONWithoutBridgedFlagDecodes() throws {
        ...
        let decoded = try JSONDecoder().decode(PilltalkMessage.self, from: JSONSerialization.data(withJSONObject: json))
        #expect(!decoded.isBridged)
    }
