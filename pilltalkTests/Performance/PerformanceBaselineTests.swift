...
// no Tor, no sleeps.
//
// Skippable via the PILLTALK_SKIP_PERF_BASELINES=1 environment variable;
// runs by default.
//
...
final class PerformanceBaselineTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        try XCTSkipIf(
            ProcessInfo.processInfo.environment["PILLTALK_SKIP_PERF_BASELINES"] == "1",
            "Performance baselines skipped via PILLTALK_SKIP_PERF_BASELINES"
        )
    }
    ...
    /// appended to the file named by `PILLTALK_PERF_LOG` (if set): under
    ...
    private static var perfLogPath: String? {
        let path = ProcessInfo.processInfo.environment["PILLTALK_PERF_LOG"]
        ...
        measure {
            ...
            for (index, packet) in packets.enumerated() {
                guard let data = packet.toBinaryData(),
                      let decoded = PilltalkPacket.from(data) else { continue }
                decodedCount += 1
    ...
    func testMessageFormatting() {
        ...
        let contents: [(String, [String]?)] = [
            ...
            ("checking in from the harbor #pilltalk #mesh", nil),
        ...
        let batches: [[PilltalkMessage]] = (0..<batchCount).map { batch in
    ...
    func testPipelinePrivateIngest() {
        ...
        let messages: [PilltalkMessage] = (0..<messageCount).map { i in
            PilltalkMessage(
                id: "perf-dm-\(i)",
                sender: "perfsender",
    ...
    private static func makeSignedGeohashEvents(count: Int) throws -> [NostrEvent] {
        ...
        let lines = [
            ...
            "check this out https://example.com/p/123 #pilltalk",
...
private final class PerfNostrContext: ChatNostrContext {
    ...
    func appendGeohashMessageIfAbsent(_ message: PilltalkMessage, toGeohash geohash: String) -> Bool { true }

    private(set) var handledPublicMessageCount = 0
    func handlePublicMessage(_ message: PilltalkMessage, powBits: Int) { handledPublicMessageCount += 1 }
...
private final class PerfFormattingContext: MessageFormattingContext {
    ...
    func isSelfMessage(_ message: PilltalkMessage) -> Bool { false }
    func senderColor(for message: PilltalkMessage, isDark: Bool) -> Color { .blue }
    func peerURL(for peerID: PeerID) -> URL? { URL(string: "pilltalk://peer/\(peerID.id)") }
}
