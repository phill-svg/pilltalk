...
struct PrekeyEndToEndTests {
    ...
    private final class PacketTap {
        ...
        func first(ofType type: MessageType) -> PilltalkPacket? {
            lock.lock(); defer { lock.unlock() }
            return packets.first { $0.type == type.rawValue }
        }
    }

    private final class NoiseCaptureDelegate: PilltalkDelegate {
        private let lock = NSLock()
        ...
        // Unused PilltalkDelegate requirements.
    ...
    private func captureAnnounceAndBundle(from peer: BLEService, tap: PacketTap) async throws -> (announce: PilltalkPacket, bundle: PilltalkPacket) {
    ...
    @Test func forgedBundleSignatureIsRejected() async throws {
        ...
        let forgedPacket = PilltalkPacket(
