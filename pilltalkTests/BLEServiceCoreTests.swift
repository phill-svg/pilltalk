...
struct BLEServiceCoreTests {
    ...
    @Test
    func modifiedServices_rediscoverWhenPillTalkServiceIsInvalidated() async throws {
        let otherService = CBUUID(string: "0000180F-0000-1000-8000-00805F9B34FB")

        #expect(BLEService._test_shouldRediscoverPillTalkService(
    ...
    @Test
    func modifiedServices_rediscoverWhenCachedServicesNoLongerIncludePillTalk() async throws {
        let otherService = CBUUID(string: "0000180F-0000-1000-8000-00805F9B34FB")
    ...
    @Test
    func modifiedServices_ignoreUnrelatedInvalidationWhenPillTalkIsStillCached() async throws {
        let otherService = CBUUID(string: "0000180F-0000-1000-8000-00805F9B34FB")
...
private final class OutboundPacketTap {
    ...
    private var packets: [PilltalkPacket] = []

    func record(_ packet: PilltalkPacket) {
...
private func makePublicPacket(content: String, sender: PeerID, timestamp: UInt64) -> PilltalkPacket {
    PilltalkPacket(
...
private func makeLeavePacket(sender: PeerID, marker: String) -> PilltalkPacket {
    PilltalkPacket(
...
private final class PublicCaptureDelegate: PilltalkDelegate {
    private let lock = NSLock()
    private(set) var publicMessages: [PilltalkMessage] = []
    ...
    func publicMessagesSnapshot() -> [PilltalkMessage] {
        lock.lock()
        defer { lock.unlock() }
