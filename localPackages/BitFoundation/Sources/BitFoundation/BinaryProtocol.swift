...
/// # BinaryProtocol
///
/// Low-level binary encoding and decoding for PillTalk protocol messages.
...
/// Implements binary encoding and decoding for PillTalk protocol messages.
/// Provides static methods for converting between PilltalkPacket objects and
...
public struct BinaryProtocol {
    ...
    // Encode PilltalkPacket to binary format
    static func encode(_ packet: PilltalkPacket, padding: Bool = true) -> Data? {
    ...
    // Decode binary data to PilltalkPacket
    public static func decode(_ data: Data) -> PilltalkPacket? {
    ...
    private static func decodeCore(_ raw: Data) -> PilltalkPacket? {
        guard raw.count >= v1HeaderSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> PilltalkPacket? in
