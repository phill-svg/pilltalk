...
final class GossipSyncManager {
    ...
    private struct PacketStore {
        private(set) var packets: [String: PilltalkPacket] = [:]
        private(set) var order: [String] = []

        mutating func insert(idHex: String, packet: PilltalkPacket, capacity: Int) {
        ...
        func allPackets(isFresh: (PilltalkPacket) -> Bool) -> [PilltalkPacket] {
        ...
        mutating func remove(where shouldRemove: (PilltalkPacket) -> Bool) {
        ...
        mutating func removeExpired(isFresh: (PilltalkPacket) -> Bool) {
            remove { !isFresh($0) }
        }
    }
    ...
    var boardPacketsProvider: (() -> [PilltalkPacket])?
    ...
    private var latestAnnouncementByPeer: [PeerID: PilltalkPacket] = [:]
    ...
    private var latestPrekeyBundleByPeer: [PeerID: (id: String, packet: PilltalkPacket)] = [:]
    ...
    func onPublicPacketSeen(_ packet: PilltalkPacket) {
        queue.async { [weak self] in
    ...
    private func isPacketFresh(_ packet: PilltalkPacket) -> Bool {
    ...
    private func isAnnouncementFresh(_ packet: PilltalkPacket) -> Bool {
    ...
    private func _onPublicPacketSeen(_ packet: PilltalkPacket) {
    ...
    private func restoreArchivedMessages() {
        ...
        for data in archive.load() {
            guard let packet = PilltalkPacket.from(data),
    ...
    /// sync queue.
    func collectPublicMessagePackets(completion: @escaping ([PilltalkPacket]) -> Void) {
        queue.async { [weak self] in
            guard let self else {
                completion([])
                return
