...
final class NostrTransport: Transport, @unchecked Sendable {
    ...
    // Provide BLE short peer ID for PillTalk embedding
    ...
    final class AckPacer: @unchecked Sendable {
        ...
        private let queue = DispatchQueue(label: "chat.pilltalk.nostr-ack-pacer")
    ...
    func sendPrivateMessage(_ content: String, to peerID: PeerID, recipientNickname: String, messageID: String) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedPillTalk.encodePMForNostr(content: content, messageID: messageID, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
    ...
    func sendFavoriteNotification(to peerID: PeerID, isFavorite: Bool) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedPillTalk.encodePMForNostr(content: content, messageID: UUID().uuidString, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
...
extension NostrTransport {
    ...
    func sendPrivateMessageGeohash(content: String, toRecipientHex recipientHex: String, from identity: NostrIdentity, messageID: String) {
        Task { @MainActor in
            ...
            guard let embedded = NostrEmbeddedPillTalk.encodePMForNostrNoRecipient(content: content, messageID: messageID, senderPeerID: senderPeerID) else {
    ...
    private func sendAckItem(_ item: QueuedAck) {
        Task { @MainActor in
            ...
            case .readDirect(let receipt, let peerID):
                ...
                guard let ack = NostrEmbeddedPillTalk.encodeAckForNostr(type: .readReceipt, messageID: receipt.originalMessageID, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
            ...
            case .deliveredDirect(let messageID, let peerID):
                ...
                guard let ack = NostrEmbeddedPillTalk.encodeAckForNostr(type: .delivered, messageID: messageID, recipientPeerID: peerID, senderPeerID: senderPeerID) else {
            ...
            case .deliveredGeohash(let messageID, let recipientHex, let identity):
                ...
                guard let embedded = NostrEmbeddedPillTalk.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID) else { return }
                sendWrappedMessage(content: embedded, recipientHex: recipientHex, senderIdentity: identity, registerPending: true)
            ...
            case .readGeohash(let messageID, let recipientHex, let identity):
                ...
                guard let embedded = NostrEmbeddedPillTalk.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID) else { return }
                sendWrappedMessage(content: embedded, recipientHex: recipientHex, senderIdentity: identity, registerPending: true)
            }
        }
    }
