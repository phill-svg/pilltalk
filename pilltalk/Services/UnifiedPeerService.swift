...
final class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [PilltalkPeer] = []
    @Published private(set) var connectedPeerIDs: Set<PeerID> = []
    @Published private(set) var favorites: [PilltalkPeer] = []
    @Published private(set) var mutualFavorites: [PilltalkPeer] = []
    
    // MARK: - Private Properties
    
    private var peerIndex: [PeerID: PilltalkPeer] = [:]
    ...
    private func updatePeers() {
        ...
        var enrichedPeers: [PilltalkPeer] = []
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: PilltalkPeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [PilltalkPeer] = []
        var mutualsList: [PilltalkPeer] = []
        var newIndex: [PeerID: PilltalkPeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by peerID: PeerID) -> PilltalkPeer? {
