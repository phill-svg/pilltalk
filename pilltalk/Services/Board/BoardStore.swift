...
final class BoardStore {
    ...
    private let queue = DispatchQueue(label: "chat.pilltalk.board.store")
    ...
    @discardableResult
    func ingest(_ wire: BoardWire, packet: PilltalkPacket) -> BoardIngestResult {
    ...
    func syncCandidates() -> [PilltalkPacket] {
        let nowMs = currentMs()
        return queue.sync {
            pruneExpiredLocked(nowMs: nowMs)
    ...
    private func ingestPostLocked(_ post: BoardPostPacket, packet: PilltalkPacket, rawPacket: Data, nowMs: UInt64) -> BoardIngestResult {
    ...
    private func loadFromDisk() {
        ...
        queue.sync {
            for entry in payloads {
                guard let packet = PilltalkPacket.from(entry.packet),
