...
/// peer-to-peer communication in PillTalk. Acts as the bridge between the transport
...
/// Manages end-to-end encryption for PillTalk using the Noise Protocol Framework.
...
final class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.pilltalk.noise.service", attributes: .concurrent)
    ...
    private static let courierPrologue = Data("pilltalk-courier-v1".utf8)
    ...
    private static let prekeyProloguePrefix = Data("pilltalk-prekey-v1".utf8)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = Data("pilltalk-announce-v1".utf8)
    ...
    /// Sign a PilltalkPacket using the noise private key
    func signPacket(_ packet: PilltalkPacket) -> PilltalkPacket? {
    ...
    /// Verify a PilltalkPacket signature using the provided public key
    func verifyPacketSignature(_ packet: PilltalkPacket, publicKey: Data) -> Bool {
...
/// These types wrap the underlying PillTalk protocol messages with encryption metadata.
