...
/// - Emits events exclusively via `PilltalkDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private static let centralRestorationID = "chat.pilltalk.ble.central"
    private static let peripheralRestorationID = "chat.pilltalk.ble.peripheral"
    ...
    private var pendingPrekeyBundles: [PeerID: PilltalkPacket] = [:]
    ...
    var _test_onOutboundPacket: ((PilltalkPacket) -> Void)?
    ...
    weak var delegate: PilltalkDelegate?
    ...
    func stopServices() {
        ...
        var leavePacket = PilltalkPacket(
    ...
    func sendFileBroadcast(_ filePacket: PilltalkFilePacket, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendFilePrivate(_ filePacket: PilltalkFilePacket, to peerID: PeerID, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
            guard !self.isPanicSuspended else { return }
            guard let payload = filePacket.encode() else {
    ...
    private func isAcceptedIngressPayload(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
    ...
    private func recordIngressIfNew(_ packet: PilltalkPacket, link: BLEIngressLinkID, peerID: PeerID) -> Bool {
    ...
    private func broadcastPacket(_ packet: PilltalkPacket, transferId: String? = nil) {
        guard !isPanicSuspended else { return }
        // Apply route if recipient exists (centralized route application)
        let packetToSend: PilltalkPacket
    ...
    private func sendEncrypted(_ packet: PilltalkPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: PilltalkPacket, data: Data, pad: Bool) {
    ...
    private func spoolDirectedPacket(_ packet: PilltalkPacket, recipientPeerID: PeerID) {
    ...
    private func signedSenderDisplayName(for packet: PilltalkPacket, from peerID: PeerID) -> String? {
    ...
    private func decodeArchivedPublicMessage(_ packet: PilltalkPacket) -> ArchivedPublicMessage? {
    ...
    private func handleFileTransfer(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
    ...
    /// has aged out.
    private func handleLeave(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
...
extension BLEService: GossipSyncManager.Delegate {
    func sendPacket(_ packet: PilltalkPacket) {
        broadcastPacket(packet)
    }

    func sendPacket(to peerID: PeerID, packet: PilltalkPacket) {
        sendPacketDirected(packet, to: peerID)
    }

    func signPacketForBroadcast(_ packet: PilltalkPacket) -> PilltalkPacket {
...
private extension BLEService {
    static func shouldRediscoverPillTalkService(
...
extension BLEService {
    ...
    func _test_handlePacket(_ packet: PilltalkPacket, fromPeerID: PeerID, preseedPeer: Bool = true, signingPublicKey: Data? = nil) {
        if preseedPeer {
    ...
    func _test_acceptsIngress(packet: PilltalkPacket, boundPeerID: PeerID?) -> Bool {
    ...
    func _test_recordIngressIfNew(packet: PilltalkPacket, linkID: String) -> Bool {
    ...
    static func _test_shouldRediscoverPillTalkService(
        invalidatedServiceUUIDs: [CBUUID],
        cachedServiceUUIDs: [CBUUID]?
    ) -> Bool {
        shouldRediscoverPillTalkService(
...
extension BLEService: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        guard let service = services.first(where: { $0.uuid == BLEService.serviceUUID }) else {
            // Not a PillTalk peer - disconnect
    ...
    private func processNotificationPacket(_ packet: PilltalkPacket, from _: CBPeripheral, peripheralUUID: String, receivedFrom peerID: PeerID) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        let shouldRediscover = BLEService.shouldRediscoverPillTalkService(
        ...
        SecureLogger.debug("🔄 PillTalk service changed for \(peripheral.name ?? peripheral.identifier.uuidString), rediscovering", category: .session)
        peripheral.discoverServices([BLEService.serviceUUID])
    }
...
extension BLEService: CBPeripheralManagerDelegate {
    ...
    private func processDecodedCentralWrite(_ packet: PilltalkPacket, centralUUID: String, central: CBCentral) {
        let claimedSenderID = PeerID(hexData: packet.senderID)
        let context = acceptedIngressContext(
            for: packet,
            claimedSenderID: claimedSenderID,
...
extension BLEService {
    ...
    private func applyRouteIfAvailable(_ packet: PilltalkPacket, to recipient: PeerID) -> PilltalkPacket {
        let now = Date()
        ...
        let routedPacket = PilltalkPacket(
            type: packet.type,
    ...
    private func handleMeshPing(_ packet: PilltalkPacket, fromLink linkPeerID: PeerID) {
    ...
    private func handleMeshPong(_ packet: PilltalkPacket, from peerID: PeerID) {
        guard packet.recipientID == myPeerIDData else { return }
    ...
    private func forwardAlongRouteIfNeeded(_ packet: PilltalkPacket) -> Bool {
    ...
    private func markNoiseAuthenticatedIngressLink(for packet: PilltalkPacket, peerID: PeerID) {
    ...
    private func isNoiseAuthenticatedIngressLink(for packet: PilltalkPacket, peerID: PeerID) -> Bool {
    ...
    private func makeEncryptedNoisePacket(_ typedPayload: Data, to peerID: PeerID) throws -> PilltalkPacket {
    ...
    private func makeCourierPacket(_ payload: Data, to peerID: PeerID) -> PilltalkPacket {
        let packet = PilltalkPacket(
    ...
    private func handleCourierEnvelope(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func acceptCourierDeposit(_ envelope: CourierEnvelope, from peerID: PeerID, packet: PilltalkPacket) {
    ...
    /// attribute.
    private func handlePrekeyBundle(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func ingestVerifiedPrekeyBundle(_ bundle: PrekeyBundle, packet: PilltalkPacket, owner: PeerID, signingKey: Data) {
    ...
    private func drainPendingPrekeyBundles(for owner: PeerID) {
        let pending: PilltalkPacket? = collectionsQueue.sync(flags: .barrier) {
    ...
    private func handleNostrCarrier(_ packet: PilltalkPacket, from _: PeerID) {
        let senderID = PeerID(hexData: packet.senderID)
    ...
    private func handleFragment(_ packet: PilltalkPacket, from peerID: PeerID) {
        if DispatchQueue.getSpecific(key: messageQueueKey) != nil {
    ...
    private func handleReceivedPacket(_ packet: PilltalkPacket, from peerID: PeerID) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            guard let lifecycleGeneration =
    ...
    private func scheduleRelayIfNeeded(_ packet: PilltalkPacket, senderID: PeerID, messageID: String) {
    ...
    private func handleAnnounce(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func rebindLinkAfterVerifiedDirectAnnounce(_ packet: PilltalkPacket, to peerID: PeerID) {
    ...
    /// link either way.
    private func retireRedundantPeripheralLinks(_ packet: PilltalkPacket, to peerID: PeerID) {
    ...
    private func handleBoardPost(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
    ...
    private func handleRequestSync(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func handleGroupMessage(_ packet: PilltalkPacket, from _: PeerID) {
    ...
    private func handleVoiceFrame(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
    ...
    private func handleNoiseHandshake(_ packet: PilltalkPacket, from peerID: PeerID) {
    ...
    private func handleNoiseEncrypted(_ packet: PilltalkPacket, from peerID: PeerID) {
