...
struct BLENoisePacketHandlerEnvironment {
    ...
    let broadcastPacket: (PilltalkPacket) -> Void
...
final class BLENoisePacketHandler {
    ...
    @discardableResult
    func handleHandshake(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
        ...
        if PeerID(hexData: packet.recipientID) == env.localPeerID() {
            ...
            do {
                ...
                if let response = result.response {
                    // Send response
                    let responsePacket = PilltalkPacket(
    ...
    func handleEncrypted(_ packet: PilltalkPacket, from peerID: PeerID) {
        let env = environment
