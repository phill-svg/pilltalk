...
struct BLEFileTransferHandlerEnvironment {
    ...
    let verifyPacketSignature: (_ packet: PilltalkPacket, _ signingPublicKey: Data) -> Bool
    /// Resolves a display name from a verified packet signature for peers missing from the registry.
    let signedSenderDisplayName: (_ packet: PilltalkPacket, _ peerID: PeerID) -> String?
    /// Tracks the broadcast file packet for gossip sync.
    let trackPacketSeen: (PilltalkPacket) -> Void
    ...
    let deliverMessage: (PilltalkMessage) -> Void
}
...
final class BLEFileTransferHandler {
    ...
    @discardableResult
    func handle(_ packet: PilltalkPacket, from peerID: PeerID) -> Bool {
        let env = environment
        ...
        let message = PilltalkMessage(
            ...
            senderPeerID: peerID,
            // Received messages need an explicit status: PilltalkMessage
