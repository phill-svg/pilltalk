...
struct BLEIngressLinkRegistry {
    ...
    func record(for packet: PilltalkPacket) -> BLEIngressLinkRecord? {
        ingressByMessageID[Self.messageID(for: packet)]
    }

    func link(for packet: PilltalkPacket) -> BLEIngressLinkID? {
        record(for: packet)?.link
    }

    func peerID(for packet: PilltalkPacket) -> PeerID? {
    ...
    static func messageID(for packet: PilltalkPacket) -> String {
    ...
    private static func requiresDirectSenderBinding(_ packet: PilltalkPacket) -> Bool {
    ...
    static func isDirectAnnounce(_ packet: PilltalkPacket, directAnnounceTTL: UInt8) -> Bool {
        packet.type == MessageType.announce.rawValue && packet.ttl == directAnnounceTTL
    }

    private static func isSelfAuthoredSyncResponse(_ packet: PilltalkPacket) -> Bool {
