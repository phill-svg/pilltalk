...
enum BLEFileTransferPolicy {
    static func isSelfEcho(packet: PilltalkPacket, from peerID: PeerID, localPeerID: PeerID) -> Bool {
        peerID == localPeerID && packet.ttl != 0
    }

    static func deliveryPlan(packet: PilltalkPacket, localPeerID: PeerID) -> BLEFileTransferDeliveryPlan? {
...
enum BLEIncomingFileValidator {
    static func validate(payload: Data) -> Result<BLEIncomingFileAcceptance, BLEIncomingFileRejection> {
        guard let filePacket = PilltalkFilePacket.decode(payload) else {
