...
struct BLEAnnounceHandlerEnvironment {
    ...
    let verifySignature: (_ packet: PilltalkPacket, _ signingPublicKey: Data) -> Bool
    ...
    /// into "connected".
    let linkBoundToOtherPeer: (_ packet: PilltalkPacket, _ peerID: PeerID) -> Bool
...
final class BLEAnnounceHandler {
    ...
    @discardableResult
    func handle(_ packet: PilltalkPacket, from peerID: PeerID) -> BLEAnnounceHandlingResult? {
        let env = environment
        let now = env.now()
