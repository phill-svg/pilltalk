...
struct BLESelfBroadcastTracker {
    ...
    mutating func record(messageID: String, packet: PilltalkPacket, sentAt: Date) {
        entriesByDedupID[Self.dedupID(for: packet)] = Entry(messageID: messageID, sentAt: sentAt)
    }

    mutating func takeMessageID(for packet: PilltalkPacket) -> String? {
    ...
    static func dedupID(for packet: PilltalkPacket) -> String {
