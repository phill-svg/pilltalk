...
final class GroupStore: ObservableObject {
    ...
    @Published private(set) var groups: [PilltalkGroup] = []
    ...
    func group(withID groupID: Data) -> PilltalkGroup? {
        groups.first { $0.groupID == groupID }
    }

    func group(for peerID: PeerID) -> PilltalkGroup? {
    ...
    func createGroup(named name: String, creator: GroupMember) -> PilltalkGroup? {
        guard let groupID = Self.randomBytes(PilltalkGroup.groupIDLength),
              let key = Self.randomBytes(PilltalkGroup.keyLength) else { return nil }
        let group = PilltalkGroup(
            groupID: groupID,
            name: name,
            epoch: 1,
            members: [creator],
    ...
    @discardableResult
    func upsert(_ group: PilltalkGroup, key: Data) -> Bool {
        guard group.groupID.count == PilltalkGroup.groupIDLength,
              key.count == PilltalkGroup.keyLength,
              !group.members.isEmpty,
              group.members.count <= PilltalkGroup.maxMembers,
    ...
    @discardableResult
    func updateRoster(groupID: Data, members: [GroupMember]) -> PilltalkGroup? {
        guard let index = groups.firstIndex(where: { $0.groupID == groupID }),
              !members.isEmpty,
              members.count <= PilltalkGroup.maxMembers,
    ...
    func rotateKey(groupID: Data, members: [GroupMember]) -> (group: PilltalkGroup, key: Data)? {
        guard let existing = group(withID: groupID),
              let newKey = Self.randomBytes(PilltalkGroup.keyLength) else { return nil }
    ...
    private func loadFromDisk() {
        guard let fileURL,
              ...
              let stored = try? JSONDecoder().decode([PilltalkGroup].self, from: data) else {
            return
        }
