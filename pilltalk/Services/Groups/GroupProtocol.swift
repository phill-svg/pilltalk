...
struct PilltalkGroup: Codable, Equatable {
...
enum GroupRosterCoding {
    ...
    static func encode(_ members: [GroupMember]) -> Data? {
        guard members.count <= PilltalkGroup.maxMembers else { return nil }
    ...
    static func decode(_ data: Data) -> [GroupMember]? {
        guard let count = data.first, count <= UInt8(PilltalkGroup.maxMembers) else { return nil }
        var members: [GroupMember] = []
...
/// "pilltalk-group-v1" | groupID | epoch | SHA256(key) | SHA256(roster) —
...
struct GroupStatePayload: Equatable {
    ...
    static let signingDomain = Data("pilltalk-group-v1".utf8)
    ...
    static func decode(_ data: Data) -> GroupStatePayload? {
        ...
        for (type, value) in fields {
            ...
            case .groupID where value.count == PilltalkGroup.groupIDLength:
                groupID = value
            case .name:
                name = String(data: value, encoding: .utf8)
            case .key where value.count == PilltalkGroup.keyLength:
                key = value
            case .epoch:
    ...
    func verifyCreatorSignature() -> Bool {
        guard members.count <= PilltalkGroup.maxMembers,
...
enum GroupCrypto {
    static let messageSigningDomain = Data("pilltalk-group-msg-v1".utf8)
