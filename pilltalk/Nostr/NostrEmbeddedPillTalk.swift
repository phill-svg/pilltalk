...
// MARK: - PillTalk-over-Nostr Adapter

struct NostrEmbeddedPillTalk {
    /// Build a `pilltalk1:` base64url-encoded PillTalk packet carrying a private message for Nostr DMs.
    static func encodePMForNostr(content: String, messageID: String, recipientPeerID: PeerID, senderPeerID: PeerID) -> String? {
        ...
        return "pilltalk1:" + base64URLEncode(data)
    }

    /// Build a `pilltalk1:` base64url-encoded PillTalk packet carrying a delivery/read ack for Nostr DMs.
    ...
    /// Build a `pilltalk1:` ACK (delivered/read) without an embedded recipient peer ID (geohash DMs).
    ...
    /// Build a `pilltalk1:` payload without an embedded recipient peer ID (used for geohash DMs).
