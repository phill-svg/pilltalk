...
private extension MessageListView {
    ...
    func unseenEligibleCount(in messages: [PilltalkMessage]) -> Int {
    ...
    func isResendableFailedMessage(_ message: PilltalkMessage) -> Bool {
        ...
        Button {
            ...
            HStack(spacing: 4) {
                Image(systemName: "arrow.down")
                    .font(.pilltalkSystem(size: 11, weight: .semibold))
                if unseenCount > 0 {
                    Text(
                        String(
                    ...
                    .pilltalkFont(size: 12, weight: .medium)
                }
            }
            .foregroundColor(palette.primary)
    ...
    @ViewBuilder
    func messageRow(for message: PilltalkMessage) -> some View {
        Group {
    ...
    @ViewBuilder
    func systemMessageRow(_ message: PilltalkMessage) -> some View {
        Text(conversationUIModel.formatMessage(message, colorScheme: colorScheme, theme: theme))
            .fixedSize(horizontal: false, vertical: true)
            .frame(maxWidth: .infinity, alignment: .leading)
    }

    func expandWindow(ifNeededFor message: PilltalkMessage,
                      allMessages: [PilltalkMessage],
                      privatePeer: PeerID?,
    ...
    func handleOpenURL(_ url: URL) {
        guard url.scheme == "pilltalk" else { return }
        switch url.host {
        case "user":
    ...
    func conversationMessages(for privatePeer: PeerID?) -> [PilltalkMessage] {
        if let privatePeer {
