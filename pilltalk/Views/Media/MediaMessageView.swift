...
struct MediaMessageView: View {
    ...
    let media: PilltalkMessage.Media
    /// Value snapshot of the message's mutable delivery status, captured at
    /// construction (see `TextMessageView.deliveryStatus`): `PilltalkMessage`
    ...
    init(message: PilltalkMessage, media: PilltalkMessage.Media, imagePreviewURL: Binding<URL?>) {
                ...
                if message.isPrivate && conversationUIModel.isSentByCurrentUser(message),
                   let status = deliveryStatus {
                    if case .failed = status {
                        Text(verbatim: status.pilltalkDescription)
    ...
    private func mediaSendState(for deliveryStatus: DeliveryStatus?, isFromMe: Bool) -> (isSending: Bool, progress: Double?, canCancel: Bool) {
        // A received message is never in a send state: PilltalkMessage defaults
