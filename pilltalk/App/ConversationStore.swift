...
final class Conversation: ObservableObject, Identifiable {
    ...
    @Published private(set) var messages: [PilltalkMessage] = []
    ...
    func message(withID messageID: String) -> PilltalkMessage? {
    ...
    fileprivate func insert(_ message: PilltalkMessage) -> InsertResult {
    ...
    fileprivate func upsert(_ message: PilltalkMessage) -> UpsertOutcome {
    ...
    fileprivate func applyDeliveryStatus(_ status: DeliveryStatus, forMessageID messageID: String) -> Bool {
        ...
        // PilltalkMessage is a reference type; write back through the
    ...
    /// copies that share a PilltalkMessage instance: the first conversation's
    ...
    fileprivate func remove(messageID: String) -> PilltalkMessage? {
        guard let index = indexByMessageID[messageID] else { return nil }
    ...
    fileprivate func removeAll(where predicate: (PilltalkMessage) -> Bool) -> [String] {
...
enum ConversationChange {
    case appended(ConversationID, PilltalkMessage)
...
final class ConversationStore: ObservableObject {
    ...
    @discardableResult
    func append(_ message: PilltalkMessage, to id: ConversationID) -> Bool {
    ...
    func upsertByID(_ message: PilltalkMessage, in id: ConversationID) {
        let conversation = conversation(for: id)
    ...
    ///
    /// `PilltalkMessage` is a reference type, so mirrored copies sharing one
    ...
    /// message is unknown.
    @discardableResult
    func removeMessage(withID messageID: String, from id: ConversationID) -> PilltalkMessage? {
        guard let conversation = conversationsByID[id],
    ...
    func removeMessages(from id: ConversationID, where predicate: (PilltalkMessage) -> Bool) {
...
extension ConversationStore {
    ...
    func directMessagesByRoutingPeerID() -> [PeerID: [PilltalkMessage]] {
        var messagesByPeerID: [PeerID: [PilltalkMessage]] = [:]
...
extension ConversationStore {
    ...
    func _testAppendBypassingCap(_ message: PilltalkMessage, to id: ConversationID) {
        let conversation = conversation(for: id)
...
extension Conversation {
    fileprivate func _testAppendBypassingTrim(_ message: PilltalkMessage) {
        messages.append(message)
        indexByMessageID[message.id] = messages.count - 1
    }
}
