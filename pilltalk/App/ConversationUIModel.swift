...
final class ConversationUIModel: ObservableObject {
    ...
    func resendFailedPrivateMessage(_ message: PilltalkMessage) {
    ...
    func formatMessage(_ message: PilltalkMessage, colorScheme: ColorScheme, theme: AppTheme? = nil) -> AttributedString {
    ...
    func mediaAttachment(for message: PilltalkMessage) -> PilltalkMessage.Media? {
    ...
    func isSentByCurrentUser(_ message: PilltalkMessage) -> Bool {
        message.sender == currentNickname || message.sender.hasPrefix(currentNickname + "#")
    }

    func isMediaMessageFromCurrentUser(_ message: PilltalkMessage) -> Bool {
        message.sender == currentNickname || message.senderPeerID == chatViewModel.meshService.myPeerID
    }

    func senderDisplayName(for peerID: PeerID, fallbackMessages: [PilltalkMessage]) -> String? {
    ...
    func isLiveVoiceMessage(_ message: PilltalkMessage) -> Bool {
