...
/// # ChatViewModel
///
/// The central business logic and state management component for PillTalk.
...
/// SwiftUI views and the underlying PillTalk services. It manages:
...
/// - **PilltalkDelegate**: Receives messages and events from BLEService
...
/// Manages the application state and business logic for PillTalk.
/// Acts as the primary coordinator between UI components and backend services,
/// implementing the PilltalkDelegate protocol to handle network events.
final class ChatViewModel: ObservableObject, PilltalkDelegate, TransportEventDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    private var visibleMessagesCache: [PilltalkMessage]?
    ...
    @Published var allPeers: [PilltalkPeer] = []
    ...
    /// form wins.
    @MainActor
    var privateChats: [PeerID: [PilltalkMessage]] {
    ...
    var peerIndex: [PeerID: PilltalkPeer] = [:]
    ...
    private let nicknameKey = "pilltalk.nickname"
    ...
    static func defaultReadReceiptsDefaults() -> UserDefaults {
        ...
        let suiteName = "chat.pilltalk.tests.readReceipts.\(UUID().uuidString)"
    ...
    @MainActor
    @discardableResult
    func appendPrivateMessage(_ message: PilltalkMessage, to peerID: PeerID) -> Bool {
    ...
    @MainActor
    func upsertPrivateMessage(_ message: PilltalkMessage, in peerID: PeerID) {
    ...
    @MainActor
    func privateMessages(for peerID: PeerID) -> [PilltalkMessage] {
    ...
    @MainActor
    @discardableResult
    func removePrivateMessage(withID messageID: String) -> PilltalkMessage? {
        var removed: PilltalkMessage?
    ...
    @MainActor
    @discardableResult
    func appendPublicMessage(_ message: PilltalkMessage, to conversationID: ConversationID) -> Bool {
    ...
    @MainActor
    @discardableResult
    func appendGeohashMessageIfAbsent(_ message: PilltalkMessage, toGeohash geohash: String) -> Bool {
        conversations.append(message, to: .geohash(geohash.lowercased()))
    }

    /// A public (mesh/geohash) channel's full timeline.
    @MainActor
    func publicMessages(for channel: ChannelID) -> [PilltalkMessage] {
    ...
    @MainActor
    @discardableResult
    func removePublicMessage(withID messageID: String) -> PilltalkMessage? {
    ...
    @MainActor
    func removePublicMessages(fromGeohash geohash: String, where predicate: (PilltalkMessage) -> Bool) {
    ...
    /// Sends a message through the PillTalk network.
    ...
    func addLocalPrivateSystemMessage(_ content: String, to peerID: PeerID) {
        let systemMessage = PilltalkMessage(
            sender: "system",
            content: content,
            timestamp: Date(),
            isRelay: false,
    ...
    func panicClearAllData(restartServices: Bool = true) -> Bool {
        ...
        userDefaults.removeObject(forKey: "pilltalk.noiseIdentityKey")
        userDefaults.removeObject(forKey: "pilltalk.messageRetentionKey")
    ...
    @MainActor
    func formatMessageAsText(_ message: PilltalkMessage, colorScheme: ColorScheme, theme: AppTheme? = nil) -> AttributedString {
        messageFormatter.formatMessageAsText(message, colorScheme: colorScheme, theme: theme ?? currentTheme)
    }

    @MainActor
    func formatMessageHeader(_ message: PilltalkMessage, colorScheme: ColorScheme, theme: AppTheme? = nil) -> AttributedString {
    ...
    @MainActor
    private func peerColor(for message: PilltalkMessage, isDark: Bool) -> Color {
    ...
    @MainActor
    func isSelfMessage(_ message: PilltalkMessage) -> Bool {
        messageFormatter.isSelfMessage(message)
    }

    @MainActor
    func senderColor(for message: PilltalkMessage, isDark: Bool) -> Color {
    ...
    func getPeer(byID peerID: PeerID) -> PilltalkPeer? {
    ...
    func didReceiveMessage(_ message: PilltalkMessage) {
    ...
    @MainActor
    func handlePublicMessage(_ message: PilltalkMessage) {
