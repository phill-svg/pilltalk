...
final class ChatMessageFormatter {
    ...
    func formatMessageAsText(_ message: PilltalkMessage, colorScheme: ColorScheme, theme: AppTheme = .matrix) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            senderStyle.font = .pilltalkSystem(size: 14, weight: fontWeight, design: design)
            if let spid = message.senderPeerID,
               let url = URL(string: "pilltalk://user/\(spid.toPercentEncoded())") {
            ...
            if content.isOversizedForRichFormatting() {
                ...
                plainStyle.font = isSelf
                    ? .pilltalkSystem(size: 14, weight: .bold, design: design)
                    : .pilltalkSystem(size: 14, design: design)
                ...
                for (range, type) in allMatches {
                    ...
                    if type == "mention" {
                        ...
                        mentionStyle.font = .pilltalkSystem(
                            size: 14,
                        ...
                        if isGeohash && !attachedToMentionToken && standalone,
                           let url = URL(string: "pilltalk://geohash/\(token)") {
                            tagStyle.link = url
                        ...
                        matchStyle.font = .pilltalkSystem(
                            size: 14,
                            weight: isSelf ? .bold : .semibold,
                            design: design
                        )
            ...
            timestampStyle.font = .pilltalkSystem(size: 10, design: design)
            result.append(timestamp.mergingAttributes(timestampStyle))
        } else {
            var contentStyle = AttributeContainer()
            contentStyle.foregroundColor = Color.gray
            let content = AttributedString("* \(message.content) *")
            contentStyle.font = .pilltalkSystem(size: 12, design: design).italic()
    ...
    func formatMessageHeader(_ message: PilltalkMessage, colorScheme: ColorScheme, theme: AppTheme = .matrix) -> AttributedString {
        let design = theme.bodyFontDesign
        let isSelf: Bool = {
            if let spid = message.senderPeerID {
        ...
        if message.sender == "system" {
            ...
            style.font = .pilltalkSystem(size: 14, weight: .medium, design: design)
        ...
        var senderStyle = AttributeContainer()
        senderStyle.foregroundColor = baseColor
        senderStyle.font = .pilltalkSystem(size: 14, weight: isSelf ? .bold : .medium, design: design)
        if let spid = message.senderPeerID,
           let url = URL(string: "pilltalk://user/\(spid.id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid.id)") {
            senderStyle.link = url
        }
    ...
    func peerURL(for peerID: PeerID) -> URL? {
        URL(string: "pilltalk://user/\(peerID.toPercentEncoded())")
    }
...
private extension ChatMessageFormatter {
    func peerColor(for message: PilltalkMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID {
