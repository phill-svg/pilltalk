...
protocol ChatLifecycleContext: AnyObject {
    ...
    var messages: [PilltalkMessage] { get }
...
final class ChatLifecycleCoordinator {
    ...
    func getMessages(for peerID: PeerID?) -> [PilltalkMessage] {
        guard let peerID else { return context.messages }
        return getPrivateChatMessages(for: peerID)
    }

    func getPrivateChatMessages(for peerID: PeerID) -> [PilltalkMessage] {
        var combined: [PilltalkMessage] = []
        ...
        var bestByID: [String: PilltalkMessage] = [:]
