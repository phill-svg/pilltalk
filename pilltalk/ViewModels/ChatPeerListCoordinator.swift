...
protocol ChatPeerListContext: AnyObject {
    ...
    // MARK: Peers & sessions
    var unifiedPeers: [PilltalkPeer] { get }
    ...
    // MARK: Notifications
    /// Posts the "pilltalkters nearby" local notification.
...
final class ChatPeerListCoordinator: @unchecked Sendable {
    ...
    // The "pilltalkters nearby" notification only fires on the transition from
...
private extension ChatPeerListCoordinator {
    ...
    func handleNetworkAvailability(_ meshPeers: [PeerID]) {
        ...
        if Date().timeIntervalSince(lastNetworkNotificationTime) >= notificationCooldownSeconds {
            ...
            SecureLogger.info(
                "👥 Sent pilltalkters nearby notification for \(meshPeers.count) mesh peers (new: \(newPeers.count))",
                category: .session
            )
        }
