...
final class NostrInboundPipeline {
    ...
    func subscribeGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        let packet = Self.decodeEmbeddedPillTalkPacket(from: content),
    ...
    func handleGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard let packet = Self.decodeEmbeddedPillTalkPacket(from: content),
              packet.type == MessageType.noiseEncrypted.rawValue,
    ...
    func processNostrMessage(_ giftWrap: NostrEvent) async {
        ...
        do {
            ...
            if content.hasPrefix("pilltalk1:") {
                let packet: PilltalkPacket? = await MainActor.run {
                    Self.decodeEmbeddedPillTalkPacket(from: content)
                }
                guard let packet else {
                    SecureLogger.error("Failed to decode embedded PillTalk packet from Nostr DM", category: .session)
                    return
                }
...
private extension NostrInboundPipeline {
    @MainActor
    static func decodeEmbeddedPillTalkPacket(from content: String) -> PilltalkPacket? {
        guard content.hasPrefix("pilltalk1:") else { return nil }
        let encoded = String(content.dropFirst("pilltalk1:".count))
        ...
        return PilltalkPacket.from(packetData)
    }
}
