...
protocol ChatPrivateConversationContext: AnyObject {
    ...
    var privateChats: [PeerID: [PilltalkMessage]] { get }
...
final class ChatPrivateConversationCoordinator {
    ...
    func addMessageToPrivateChatsIfNeeded(_ message: PilltalkMessage, targetPeerID: PeerID) {
    ...
    func mirrorToEphemeralIfNeeded(_ message: PilltalkMessage, targetPeerID: PeerID, key: Data?) {
        guard let key,
    ...
    func migratePrivateChatsIfNeeded(for peerID: PeerID, senderNickname: String) {
        ...
        if context.privateMessages(for: peerID).isEmpty {
            ...
            var partiallyMigratedMessages: [PilltalkMessage] = []
