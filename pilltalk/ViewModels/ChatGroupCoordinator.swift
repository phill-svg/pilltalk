...
final class ChatGroupCoordinator {
    ...
    func listGroups() -> CommandResult {
        ...
        let lines = groups.map { group -> String in
            ...
            return "#\(group.name)\(role) — \(group.members.count)/\(PilltalkGroup.maxMembers)"
        }
...
private extension ChatGroupCoordinator {
    ...
    func selectedGroup() -> PilltalkGroup? {
    ...
    func signedStatePayload(for group: PilltalkGroup, key: Data) -> Data? {
    ...
    func distributeState(_ payload: Data, group: PilltalkGroup, excluding excludedFingerprints: Set<String>, type: StateSendType) {
        let myFingerprint = context.myNoiseFingerprint()
    ...
    func notifyRemovedMember(_ removed: GroupMember, rotated: PilltalkGroup) {
        guard let peerID = context.connectedPeerID(forFingerprint: removed.fingerprint) else { return }
        let throwawayKey = Data(count: PilltalkGroup.keyLength)
